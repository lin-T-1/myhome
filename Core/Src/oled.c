...
/* 软件 I2C 运行时引脚与地址（初始化时自动扫描确定） */
static GPIO_TypeDef *s_scl_port = GPIOB;
static GPIO_TypeDef *s_sda_port = GPIOB;
static uint16_t s_scl_pin = GPIO_PIN_8;
static uint16_t s_sda_pin = GPIO_PIN_9;
...
static void OLED_SW_Delay(void)
{
	volatile uint32_t i;
	for (i = 0; i < 100; i++)
	{
...
static void OLED_SW_SetPinOutput(uint16_t pin)
{
...
static void OLED_SW_ConfigPins(uint16_t scl_pin, uint16_t sda_pin)
{
	__HAL_RCC_GPIOB_CLK_ENABLE();
	s_scl_pin = scl_pin;
	s_sda_pin = sda_pin;
	OLED_SW_SetPinOutput(scl_pin | sda_pin);
	HAL_GPIO_WritePin(GPIOB, scl_pin | sda_pin, GPIO_PIN_SET);
}

static void OLED_W_SCL(uint8_t bit)
{
	HAL_GPIO_WritePin(s_scl_port, s_scl_pin, bit ? GPIO_PIN_SET : GPIO_PIN_RESET);
	OLED_SW_Delay();
}

static void OLED_W_SDA(uint8_t bit)
{
	HAL_GPIO_WritePin(s_sda_port, s_sda_pin, bit ? GPIO_PIN_SET : GPIO_PIN_RESET);
	OLED_SW_Delay();
}

static uint8_t OLED_I2C_ReadAck(void)
{
	uint8_t ack;
	GPIO_InitTypeDef g = {0};
	...
	OLED_W_SCL(1);
	OLED_SW_Delay();
	ack = (HAL_GPIO_ReadPin(GPIOB, s_sda_pin) == GPIO_PIN_RESET);
	OLED_W_SCL(0);

	OLED_SW_SetPinOutput(s_sda_pin);
	return ack;
}
...
static uint8_t OLED_I2C_SendByte(uint8_t byte)
{
	...
	return OLED_I2C_ReadAck();
}

static uint8_t OLED_I2C_CheckDevice(uint8_t addr)
{
	uint8_t ok;
	OLED_I2C_Start();
	ok = OLED_I2C_SendByte(addr);
	OLED_I2C_Stop();
	return ok;
}

static uint8_t OLED_I2C_BusScan(void)
{
	static const uint16_t pin_pairs[][2] = {
		{GPIO_PIN_8, GPIO_PIN_9},
		{GPIO_PIN_9, GPIO_PIN_8},
	};
	static const uint8_t addrs[] = {0x78, 0x7A};
	uint8_t p, a;
	...
	for (p = 0; p < 2; p++)
	{
		OLED_SW_ConfigPins(pin_pairs[p][0], pin_pairs[p][1]);
		for (a = 0; a < 2; a++)
		{
			if (OLED_I2C_CheckDevice(addrs[a]))
			{
				s_i2c_addr = addrs[a];
				return 1;
			}
	...
	OLED_SW_ConfigPins(GPIO_PIN_8, GPIO_PIN_9);
	s_i2c_addr = 0x78;
	return 0;
}
...
void OLED_Init(void)
{
	...
	OLED_WriteCommand(0xFF);	//对比度最大，便于确认显示
...
/* ─── 智能家居应用层显示（适配 smart_home.c） ─── */

void OLED_ShowDashboard(uint8_t has_light, uint8_t lamp_on, uint16_t light_raw,
                        uint16_t temp_raw, uint8_t temp_high)
{
	OLED_Clear();

	OLED_ShowString(0, 0, (char *)"ADC:", OLED_8X16);
	OLED_ShowNum(40, 0, light_raw, 4, OLED_8X16);

	OLED_ShowString(0, 16, (char *)"Tp ", OLED_8X16);
	OLED_ShowNum(24, 16, temp_raw, 4, OLED_8X16);

	OLED_ShowString(0, 32, (char *)"Lamp:", OLED_8X16);
	if (lamp_on)
		OLED_ShowString(48, 32, (char *)"ON", OLED_8X16);
	else
		OLED_ShowString(48, 32, (char *)"OFF", OLED_8X16);

	OLED_ShowString(0, 48, (char *)"Amb:", OLED_8X16);
	if (has_light)
		OLED_ShowString(40, 48, (char *)"BRIGHT", OLED_8X16);
	else
		OLED_ShowString(40, 48, (char *)"DARK", OLED_8X16);

	OLED_Update();
}

void OLED_ShowSensorData(uint8_t has_light, uint8_t lamp_on, uint16_t light_raw,
                         uint16_t temp_raw, uint8_t temp_high)
{
	OLED_ShowDashboard(has_light, lamp_on, light_raw, temp_raw, temp_high);
}
